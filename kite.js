...
kite_api = function () {
    ...
    const orderbook = {
        ...
        add_open_order : function(order) {
            if (order.status == "OPEN") {
                ...
                $('#open_order_list').append(`<tr id="${row_id}" ordid="${order.norenordno}" exch="${order.exch}" tsym="${order.tsym}" 
                        ...
                        <td><button type="button" class="btn btn-success modify" onclick="kite_api.orderbook.modify_order(this)">Modify</button></td>
                        <td><button type="button" class="btn btn-danger cancel" onclick="kite_api.orderbook.cancel_order(this)">Cancel</button></td>
                </tr>`);
        ...
        exit_order_cb: function(matching_order, orders, tr_elm){
            ...
            tr_elm.find('.modify').parent().html(`CLOSED</br><span class="badge badge-pill badge-secondary" title="Watch live" onclick="kite_api.trade.toggle_watch_closed_trade($(this))" style="cursor:pointer;padding:8px;margin-top:10px">Watch</span>`);
            tr_elm.find('.exit').parent().html(`<button type="button" class="btn btn-dark btn-sm" onclick="$(this).parent().parent().remove();kite_api.trade.reset_max_profit_loss()">Delete</button>`);
    ...
    const trade = {
        ...
        display_active_trade : function(order, target, sl, paper_trade=false) {
            ...
            tbody_elm.append(`<tr id="${row_id}" ordid="${order.norenordno}"  exch="${order.exch}" token="${order.token}" qty="${order.qty}" tsym="${order.tsym}" ttype="${ttype}" trtype="${order.trantype}" trade="active">
                        ...
                        <td><input type="text" class="form-control qty" placeholder=""  value="${order.qty}"></td>
                        <td><button type="button" class="btn btn-success modify" onclick="kite_api.trade.modify(this, $(this).text())">Edit</button></td>
                        <td><button type="button" class="btn btn-danger exit" onclick="kite_api.trade.exit(this)">Exit</button></td>
                </tr>`);
        ...
        exit : function(elm) {
            kite_api.orderbook.exit_order(elm);
        },
    ...
    const watch_list = {
        ...
        restore_watch_list : function() {
            ...
            for(const [key, value_str] of Object.entries(stored_entries)) {
                kite_api.watch_list.add_row_to_watch(JSON.parse(value_str))
            }
        },
        ...
        add_row_to_watch : function(params) {
            ...
            $('#watch_list_body').append(`<tr class="${class_name}" exch="${params.exch}" token="${params.token}" tsym="${params.tsym}" lot_size="${params.lot_size}" dname="${params.sym}">
                ...
                <td class="input_box"><input type="text" class="form-control entry" placeholder="" onclick="kite_api.watch_list.add_ltp(this)" ></td>
                <td class="input_box"><input type="text" class="form-control qty" placeholder="" value="${params.lot_size}"></td>
                <td><button type="button" class="btn btn-success buy" onclick="kite_api.orderbook.buy(this)">BUY</button></td>
                <td><button type="button" class="btn btn-danger sell" onclick="kite_api.orderbook.sell(this)">SELL</button></td>
                <th class="del-icon" onclick="kite_api.watch_list.delete_item(this)">
    ...
    const positions = {
        ...
        show_positions : function() {
            ...
            this.get_positions(function(positions) {
                ...
                if (positions != undefined && positions.stat !== 'Not_Ok')
                    positions.forEach((position)=> kite_api.positions.show_position(position, pnl))
